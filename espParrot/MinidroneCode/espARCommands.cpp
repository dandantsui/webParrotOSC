...
void ARCommands::sendPCMD(int8_t roll, int8_t pitch, int8_t yaw, int8_t gaz, uint8_t rollPitchFlag) {
  // if (_mdState.flyingState != FLYING_STATE_FLYING && _mdState.flyingState != FLYING_STATE_HOVERING) {
  //   Serial.print("PCMD FAIL. not flying or hoverin state");
  //   return;
  // }
...
void ARCommands::sendFlipAnimation(int8_t direction) {
  const uint8_t length = 8;
  uint8_t toSend[length];
  toSend[0] = PROJECT_MINIDRONE;
  toSend[1] = MD_ANIMATIONS;
  _uint16tToBuffer(0, &toSend[2]);
  // _uint16tToBuffer(0, &toSend[3]);
  toSend[4] = direction;  // 0 for front flip, 1 for back flip, 2 for right flip, 3 for left flip
...
// 2/1/1
// minidrone/speedSettings/max=98alSpeed
